...
then
  ...
  then
    ...
    then
      ...
      echo $RESULT | while IFS="|" read ATOMIC_NUMBER NAME SYMBOL TYPE ATOMIC_MASS MELTINGPOINT BOILINGPOINT
      do
        echo "The element with atomic number $ATOMIC_NUMBER is $NAME ($SYMBOL). It's a $TYPE, with a mass of $ATOMIC_MASS amu. $NAME has a melting point of $MELTINGPOINT celsius and a boiling point of $BOILINGPOINT celsius."
      done
    fi
  else
